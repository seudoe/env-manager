...
export default function ProjectLayout({
  ...
  const tabs = [
    ...
    {
      href: `/user/project/${projectId}/contributors`,
      label: "Contributors",
    ...
    {
      href: `/user/project/${projectId}/settings`,
      label: "Settings",
  ...
  const isActive = (href: string) => {
    if (href === `/user/project/${projectId}`) return pathname === href;
  ...
  return (
    <div>
      {/* Tab bar */}
      <div className="flex items-end gap-0 border-b border-border-subtle mb-6 -mt-2">
        {tabs.map((tab) => {
          ...
          return (
            <Link
              key={tab.href}
              href={tab.href}
              className={`flex items-center gap-1.5 px-3 py-2.5 text-[13px] font-medium border-b-2 transition-all duration-150 -mb-px ${
                active
